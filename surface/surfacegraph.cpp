...
const int sampleCountX = 120;
const int sampleCountZ = 100;
const int heightMapGridStepX = 6;
const int heightMapGridStepZ = 6;
const float sampleMin = 0.f;
const float sampleMax = 120.f;

const int sampCount = 120;  //x
const int sampMin = -1.0f;
const int sampMax = 4.0f;
const int specCount = 100;  //z
    ...
    : m_graph(surface)
{
  ...
  //  fillSqrtSinProxy();
    ...
    //! [2]
    QImage heightMapImage(":/new/mountain");
...
void SurfaceGraph::RefreshData()
{
    float stepX = 1;

    QSurfaceDataArray *dataArray = new QSurfaceDataArray;
    dataArray->reserve(sampCount);

    getSpecData();//get data to m_specData;

    int x=1;
    for(QFixList<float*>::iterator iter=m_specData.begin();iter!=m_specData.end();iter++)
    {
        QSurfaceDataRow *newRow = new QSurfaceDataRow(specCount);
        float *specData=*iter;
        int index = 0;

        for (int j = 0; j < specCount; j++) {
            float z = qMin((float)specCount, (j * stepX + 1));
            float y = *(specData+j);
            (*newRow)[index++].setPosition(QVector3D(x, y, z));
        }
        *dataArray << newRow;
...
void SurfaceGraph::enableSqrtSinModel(bool enable)
{
    if (enable) {
        ...
        m_graph->axisX()->setRange(0, 120);
        m_graph->axisY()->setRange(-1.f, 4.0f);
        ...
        m_rangeMinZ = 0;//sampleMin;
        m_stepX =1;//(sampleMax - sampleMin) / float(sampleCountX - 1);
        m_stepZ =1;//(sampleMax - sampleMin) / float(sampleCountZ - 1);
        m_axisMinSliderX->setMaximum(sampleCountX - 1);
        m_axisMinSliderX->setValue(0);
        m_axisMaxSliderX->setMaximum(sampleCountX - 0);
        m_axisMaxSliderX->setValue(sampleCountX -0);
        m_axisMinSliderZ->setMaximum(sampleCountZ - 1);
        m_axisMinSliderZ->setValue(0);
        m_axisMaxSliderZ->setMaximum(sampleCountZ - 0);
        m_axisMaxSliderZ->setValue(sampleCountZ - 0);
        //! [8]
    }
}
...
void SurfaceGraph::getSpecData()
{
    float *spec=new float[100];
    qsrand(QTime(0,0,0).secsTo(QTime::currentTime()));
    //xAxis is t; y is specdata; z is specunit;
    for(int i=0;i<100;i++)
    {

    *(spec+i)=(qrand()%400)/100;
    }
    m_specData.enList(spec);
